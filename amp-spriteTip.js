...
(function () {
    amp.plugin("spriteTip", function (options) {
        ...
        player.addEventListener(amp.eventName.loadeddata, function () {
            GetThumbnailTrack(player, options.vttUrl, options.spriteImgUrl);
        });
    });
}).call(this);
function GetThumbnailTrack(player, vttUrl, spriteImgUrl) {
    var xhr = new XMLHttpRequest();
    xhr.onload = function () {
        var vttData = xhr.responseText;
        ParseThumbnailTrack(player, vttUrl, vttData, spriteImgUrl);
    };
    xhr.open("GET", vttUrl, true);
    xhr.send();
}
function ParseThumbnailTrack(player, vttUrl, vttData, spriteImgUrl) {
    var decoder = WebVTT.StringDecoder();
    var parser = new WebVTT.Parser(window, decoder);
    parser.oncue = function (cue) {
        if (cue.text.indexOf("#xywh=") > -1) {
            _imgUrl = spriteImgUrl ;
            if (_imgUrl == null) {
                ...
                console.log("Using spriteImgUrl: "+_imgUrl) ;
            }
